...
- `reference_image`: identity tensor with shape `[3, H, W]`
...
class ControllaDataset(Dataset):
    ...
    def __init__(self, config: dict[str, Any], dummy_data: bool = False, train: bool = True) -> None:
        self.config = config
        data_config = config["data"]
        self.image_size = int(data_config["image_size"])
        self.transform = build_image_transform(self.image_size)
        self.audio_dim = int(config["model"]["audio_dim"])
        self.num_emotions = int(config["model"]["num_emotions"])
        ...
        emotion_taxonomy = [str(label).strip().lower() for label in data_config.get("emotion_taxonomy", [])]
        self.emotion_to_index = {label: index for index, label in enumerate(emotion_taxonomy)}
        self.rows = []
    ...
    def _resolve_base_path(self, raw_path: str | None, project_root: Path) -> Path | None:
        if not raw_path:
            return None
        ...
        return [
            {
                ...
                self.emotion_column: str(index % self.config["model"]["num_emotions"]),
                self.audio_column: "",
            }
    ...
    def _resolve_path(self, raw_path: str) -> Path | None:
        ...
        if self.image_root is not None:
            candidate = self.image_root / raw_path
            if candidate.exists():
                return candidate
        return None

    def _load_image_or_dummy(self, image_path: Path | None, seed: int) -> torch.Tensor:
        if image_path is None:
            generator = torch.Generator().manual_seed(seed)
            return torch.rand((3, self.image_size, self.image_size), generator=generator) * 2.0 - 1.0
        image = Image.open(image_path).convert("RGB")
        return self.transform(image)

    def _load_audio_or_dummy(self, audio_path: Path | None, seed: int) -> tuple[torch.Tensor, float]:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        reference_image = self._load_image_or_dummy(reference_path, seed=index + 1000)
        audio_features, has_audio = self._load_audio_or_dummy(audio_path, seed=index + 2000)

        return {
            "prompt": row.get(self.prompt_column, "portrait"),
            "image": image,
            "reference_image": reference_image,
            "emotion_label": torch.tensor(self._encode_emotion_label(row.get(self.emotion_column, 0)), dtype=torch.long),
            "audio_features": audio_features,
            "has_reference": torch.tensor(float(reference_path is not None), dtype=torch.float32),
...
def build_dataloader(config: dict[str, Any], train: bool = True, dummy_data: bool = False) -> DataLoader:
    """Construct a dataloader for train or evaluation runs."""
    dataset = DummyControllaDataset(config) if dummy_data else ControllaDataset(config, train=train)
